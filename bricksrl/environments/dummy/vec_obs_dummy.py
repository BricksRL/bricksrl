...
class VecGoalObsDummyEnv(EnvBase):
    ...
    def _reset(self, tensordict: TensorDictBase, **kwargs) -> TensorDictBase:
        ...
        self.episode_step_iter = 0
        observation = self.observation_spec[self.observation_key].rand()
        goal_observation = self.observation_spec[self.goal_observation_key].rand()
        return TensorDict(
            {
                self.observation_key: observation.float(),
                self.goal_observation_key: goal_observation.float(),
            },
            batch_size=[1],
        )
    ...
    def _step(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        action = tensordict.get("action").cpu().numpy()
        next_observation = self.observation_spec[self.observation_key].rand()
        goal = tensordict.get(self.goal_observation_key)
        ...
        next_tensordict = TensorDict(
            {
                self.observation_key: next_observation.float(),
                self.goal_observation_key: goal.float(),
                "reward": torch.tensor([reward]).float(),
                "done": torch.tensor([done]).bool(),
            },
            batch_size=[1],
