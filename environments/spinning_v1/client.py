...
while True:
    ...
    left_motor.run_angle(speed=400, rotation_angle=transform_range(left_action_value, -1, 1, -100, 100), wait=False)
    right_motor.run_angle(speed=400, rotation_angle=transform_range(right_action_value, -1, 1, -100, 100), wait=False)
    ...
    # send current state
    out_msg = ustruct.pack('!fffff', normalize_angle(left), normalize_angle(right), pitch, roll, z_angl_vel)
